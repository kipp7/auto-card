...
> 说明：所有表默认使用 `utf8mb4`，并为字段添加中文注释。业务字段使用 `snake_case`，API 对外字段使用 `camelCase`（见 `docs/c-api`、`docs/b-api`）。
...
CREATE TABLE `products` (
  ...
  `promo_price` decimal(10,2) DEFAULT NULL COMMENT '促销价',
  `promo_start_at` datetime DEFAULT NULL COMMENT '促销开始时间',
  `promo_end_at` datetime DEFAULT NULL COMMENT '促销结束时间',
...
## 5. 卡密库存表：product_cards

> 说明：每条记录代表一份可发放的卡密。下单会**预占**一条 `available` 记录；支付成功后标记为 `sold`。
...
> 说明：用于保存营销与业务规则配置（如满减规则）。

```sql
CREATE TABLE `settings` (
  `id` int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键',
  `config_key` varchar(100) NOT NULL COMMENT '配置键',
  `config_value` mediumtext NOT NULL COMMENT '配置值(JSON字符串)',
